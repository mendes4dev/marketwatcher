...
export async function getStaticProps() {
  ...
  const allDataGoogle = await getGoogle()
  const allDataZoom = await getZoom()
  return {
    props: {
      allData,
      allDataApple,
...
export default function Home({allData, allDataApple, allDataGoogle, allDataZoom }) {
  return (
      ...
      <div className="container" style={{marginTop: "5%"}}>
        <div className="row justify-content-around">
          <div className="col-4" style={{borderRadius: "15px 50px",border: "1px solid black"}}>
              ...
              <h3 className={styles.fonted}>
                  Open Value: {allDataGoogle['1. open']} USD
              </h3>
              <h3 className={styles.fonted}>
                  Close Value: {allDataGoogle['4. close']} USD
              </h3>
          </div>
          <div className="col-4" style={{borderRadius: "50px 10px 50px 10px",border: "1px solid black"}}>
              <img src="https://pt.wizcase.com/wp-content/uploads/2020/06/Zoom-Logo.jpg" 
                width="200" height="200" className={styles.imaged}/>
              <h3 className={styles.fonted}>
                  Date: {formatDate(Date.now() - 86400000)}
              </h3>
              <h3 className={styles.fonted}>
                  Open Value: {allDataZoom['1. open']} USD
              </h3>
              <h3 className={styles.fonted}>
                  Close Value: {allDataZoom['4. close']} USD
              </h3>
          </div>
        </div>
      </div>
...
async function getGoogle() {
  ...
  const res = await fetch("https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=GOOG&apikey=IDY3SC7RCQOA1AHP")
  //, {
  //"method": "GET",
  //"headers":{
  //  'Content-type':'application/json'
  ...
  return googleData['Time Series (Daily)'][formatDate(Date.now() - 86400000)];
}

async function getZoom() {

  const res = await fetch("https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=ZM&apikey=IDY3SC7RCQOA1AHP")
  //, {
  //"method": "GET",
  //"headers":{
  //  'Content-type':'application/json'
  ...
  return zoomData['Time Series (Daily)'][formatDate(Date.now() - 86400000)];
}
